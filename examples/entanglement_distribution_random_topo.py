...
log.install(s)
topo = RandomTopology(nodes_number= nodes_number, lines_number = lines_number,
...
net.build_route()
net.random_requests(5, attr= {"send_rate": send_rate})
...
for req in net.requests:
    src = req.src
    app = src.apps[0]
    log.info(f"{req}: succ: {src.apps[0].success_count}, total: {src.apps[0].send_count}")