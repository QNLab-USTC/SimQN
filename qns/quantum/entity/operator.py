...
class Generator(Entity):
    ...
    def generate(self, polar: Polar) -> Qubit:
        ...
        q.polar = polar.value
        self.simulator.add_event(self.simulator.current_time_slice+self.delay,
                                 GenerateSEvent(self.simulator.current_time_slice+self.delay, q))
        self.source.call(self.simulator, (q,), self, RecieveEvent(
            self.simulator.current_time_slice+self.delay), self.simulator.current_time_slice+self.delay)
        ...
        if r <= self.success_prob:
            if error_model is None:
                if qubit.polar.value == 0 or 1:
                    if basis.value == 0:
                        self.simulator.add_event(self.simulator.current_time_slice+self.delay, MeasureSEvent(
                            self.simulator.current_time_slice+self.delay, qubit))
                        self.source.call(self.simulator, (qubit.polar.value,), self, RecieveEvent(
                            self.simulator.current_time_slice+self.delay), self.simulator.current_time_slice+self.delay)
                    else:
                        tmp = random.random()
                        if tmp <= 0.5:
            ...
            self.simulator.add_event(self.simulator.current_time_slice+self.delay,
                                     MeasureFEvent(self.simulator.current_time_slice+self.delay, qubit))
            self.source.call(self.simulator, (Polar.N,), self, RecieveEvent(
                self.simulator.current_time_slice+self.delay), self.simulator.current_time_slice+self.delay)
