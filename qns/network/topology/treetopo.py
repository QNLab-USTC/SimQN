...
class TreeTopology(Topology):
    ...
    """
    def __init__(self, nodes_number, children_number: int = 2, nodes_apps: List[Application] = [], qchannel_args: Dict = {}, cchannel_args: Dict = {}, memory_args: Optional[List[Dict]] = {}):
        """
        Args:
        ...
        for i in range(self.nodes_number):
            for j in range(i * self.children_number + 1, (i+1) * self.children_number + 1):
                if j < self.nodes_number:
                    l = QuantumChannel(name= f"l{i},{j}", **self.qchannel_args)
                    ll.append(l)
                    nl[i].add_qchannel(l)
                    nl[j].add_qchannel(l)
